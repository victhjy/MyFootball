...
- (void)loadImage:(id)obj
{
    if ([obj isKindOfClass:UIImage.class]) {
        ...
        [self.indicator startAnimating];
        weakify(self);
        [self.imageView sd_setImageWithURL:obj placeholderImage:nil options:0 completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            strongify(weakSelf);
            [strongSelf.indicator stopAnimating];
            if (error) {
